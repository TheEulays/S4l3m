...
class Sarkis: #Noob bot
    def __init__(self):
        ...
        self.devine = [[k,1,1,1] for  k in manche.pack ] 
        self.strategie=""
    ...
    def annonce(self,main,manche):
        ...
        for couleur in ["coeur","carreau","pique","trefle"]:
            ...
            o=self.plimaitre(main,manche)
            o.append(self.atout_main(main,manche))
            dico[couleur]=o
        for couleur in ["coeur","carreau","pique","trefle"]:
            appel[couleur]=0
            A=False
            if len(dico[couleur])>1:
                for k in dico[couleur][:-1]:
                    A= A or k.valeur=="valet"
                if A:
                    if dico[couleur][-1] in (3,4):
                        appel[couleur]=80
                    elif dico[couleur][-1]>=5:
                        appel[couleur]=100
                    appel[couleur]+=10*(len(dico[couleur])-2)
        theappel=maxplus(appel)
        if len(manche.encheresLog)>=2:
            if manche.encheresLog[-2][0]%2==self.index%2:
                if manche.encheresLog[-2]!=self.monappel:
                    manche.atout=manche.encheresLog[-2][2]
                    b=self.plimaitre(main,manche)
                
                    encherepart=manche.encheresLog[-2][1]
                    encherepart+=10*(len(b))
                    if theappel[1]<=encherepart:
                        theappel=(manche.atout,encherepart)
            manche.atout=a
        theappel=(theappel[1],theappel[0])
        if manche.encheresLog[-1][1]<theappel[0]:
            self.monappel=theappel[1]
            return(self.index,)+(theappel)
        else:
            return manche.encheresLog[-1]
    def plimaitre(self,main,manche):
        try:
            a=manche.couleurdemandee
        except:
            a="rien"
        plimaitres=[]
        for k in main:
    ...
    def jouerUneCarte(self, main, manche):
        ...
        b=[]
        cartatou=cartesALaCouleur(main,manche.atout)    
        
        
        if manche.couleurdemandee =="NoColor":
            if self.strategie=="atk" and  self.ilrestedezatouts(manche)!=0 and cartatou!=[]:
                for k in a[:-1]:
                    if k.couleur == manche.atout:
                        return k
            ...
            cartescouleur=cartesALaCouleur(main,manche.couleur)
            
            if len(manche.pliEnCours)==1:
                if manche.couleur== manche.atout:
                    if self.strategie=="atk":
                        m=max(cartatou)
                        if m>manche.pliEnCours[-1]:
                            return m
                        else:
                            return min(cartatou)
                    else:
                        c=[k for k in cartatou if k>pliEnCours[0]]
                        if c!=[]:
                            return min(c)
                        else:
                            return min(cartatou)
                    ...
                    if cartescouleur !=[]:
                        d=[k for k in cartescouleur if k  in a]
                        if d!=[]:
                            return max(d)
                        else:
                            return min(cartescouleur)
                    elif cartatou!=[]:
                        if self.strategie=="atk":
                            return min(cartatou)
                        else:
                ...
                flag=cartpart>manche.pliEnCours[-1]
                if manche.couleurdemandee!=manche.atout:
                    for k in self.paketo:
                        if k.couleur!=manche.atout and self.devine[k][(self.index+1)%4]!=0:
                            flag = flag and cartpart>k
                else:
                    for k in self.paketo:
                        if self.devine[k][(self.index+3)%4]!=0:
                            flag = flag and cartpart>k
                if flag:
                    if cartescouleur!=[]:
                        return max([k for k in cartescouleur if k not in a])
                    else:
                        return max([k for k in main if (k not in a and k not in cartatou) ])
                else:
                    if self.strategie=="atk":
                        if cartescouleur!=[]:
                            return min([k for k in cartescouleur if k not in a])
                        elif  (cartpart>manche.pliEnCours[-1] or cartatou==[]):
                            return min(main)
                        else:
                            return min(cartatou)
                    else:
                ...
                cartpart=manche.pliEnCours[-2]
                maxpli=max(manche.pliEnCours)
                flag=(cartpart==maxpli)
                if cartescouleur!=[]:
                    m=max(cartescouleur)
                    if m>maxpli:
                        return m
                    ...
                    if self.strategie=="atk":
                        if flag:
                            ...
                            e=[k for ki in cartatou if k>maxpli]
                            if e!=[]:
                                return min(e)
                            else:
                                return min(cartatou)
